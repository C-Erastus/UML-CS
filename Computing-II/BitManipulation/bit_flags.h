...
//Precondition: nuber of bits is a positive integer.
...
//	internal representaton if the flag_position is too large instead of failing for out of bounds. Returns
...
//Precondition: flag_position is a non-negative integer and hBit_flags is a handle to a valid Bit_flags object.
//Postconditon: The flag at the flag_positon index is set to 0. Function will attempt to resize the 
//	internal representation if the flag_position is too large instead of failing for out of bounds. Returns
// 	SUCCESS if the operation is successful and FAILURE if the operation fails a needed resize. This
//	operation is considered to be expensive if flag_positon is constantly going out of bounds by a small
//	amount because the resize always attempts to minimize the amount of space required to stored the bits.
//	All new flags are created in a resize operation will be set as zero.
Status bit_flags_unset_flag(BIT_FLAGS hBit_flags, int flag_position);

//Precondition: flag_positon is a non-negative integer and hBit_flags is a handle to a valid Bit_flags object
//Postcondition: returns the value of the flag at index flag_position if it is in bounds or -1 otherwise.
int bit_flags_check_flag(BIT_FLAGS hBit_flags, int flag_position);

//Preconditon: hBit_flags is a handle to a valid Bit_flags object
//Postconditoin: returns the number of bits currently held by the data structure.
int bit_flags_get_size(BIT_FLAGS hBit_flags);

//Precondition: hBit_flags is a handle to a valid Bit_flags object
//Postconditon: returns the number of bits the object can hold.
