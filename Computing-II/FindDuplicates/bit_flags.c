...
BIT_FLAGS bit_flags_init_number_of_bits(int number_of_bits){
	...
	if(pBit_flags != NULL){
		...
		pBit_flags->capacity = (number_of_bits / BITS) + 1;
...
Status bit_flags_set_flag(BIT_FLAGS hBit_flags, int flag_position){
	...
	if(flag_position >= pBit_flags->capacity * BITS){
		temp = (int*)malloc(sizeof(int) * (flag_position / BITS) + 1); //in terms of data index or in terms of regular index
		if(temp == NULL ){

			return FAILURE;
		}
		for(int i = 0; i < (flag_position / BITS) + 1; i++){
			temp[i] = 0;
		}
		...
		pBit_flags->capacity = (flag_position / BITS) +1;
		//set the bits
	}
	pBit_flags->data[flag_position / BITS] |= (1 << flag_position % BITS);
...
Status bit_flags_unset_flag(BIT_FLAGS hBit_flags, int flag_position){
	...
	if(flag_position >= pBit_flags->capacity * BITS){
		temp = (int*)malloc(sizeof(int) * (flag_position / BITS));
		if(temp == NULL){

			return FAILURE;
		}
		for(int i = 0; i < (flag_position / BITS); i++){
			temp[i] = 0;
		}
		for(int i = 0; i < pBit_flags->capacity; i++){
			temp[i] = pBit_flags->data[i];
		...
		free(pBit_flags->data);
		pBit_flags->data = temp;
		pBit_flags->capacity = (flag_position / BITS);
	}
	pBit_flags->data[flag_position / BITS] &= ~ (1 << (flag_position % BITS));
	return SUCCESS;
}
int bit_flags_check_flag(BIT_FLAGS hBit_flags, int flag_position){
	...
	val = 1 & (pBit_flags->data[flag_position / BITS] >> flag_position);
...
int bit_flags_get_capacity(BIT_FLAGS hBit_flags){
	...
	return pBit_flags->capacity * BITS;
}
